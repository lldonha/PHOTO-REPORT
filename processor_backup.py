...
from overlay_generator_v2 import aplicar_mascara_v2
...
class AplicarMascaraRequest(BaseModel):
    ...
    usar_overlay_v2: bool = True  # Por padrão usa V2 (Solocator style)
...
class MobileUploadMetadata(BaseModel):
    """Metadados da foto enviada pelo app mobile."""
    latitude: Optional[float] = None
    longitude: Optional[float] = None
    altitude: Optional[float] = None
...
class MobileUploadRequest(BaseModel):
    """Request para o endpoint /mobile/upload."""
    image_base64: str
    metadata: MobileUploadMetadata
...
class MobileUploadResponse(BaseModel):
    """Resposta do endpoint /mobile/upload."""
    success: bool
    message: str
    id: str
...
async def aplicar_mascara_endpoint(request: AplicarMascaraRequest):
    ...
    try:
        ...
        # Aplica máscara/overlay (V2 por padrão)
        if request.usar_overlay_v2:
            # Overlay V2 - Estilo Solocator com bússola visual
            imagem_processada = aplicar_mascara_v2(
                dados_imagem=dados_imagem,
                direcao_graus=request.direcao_graus,
                latitude=request.latitude,
                longitude=request.longitude,
                acuracia=request.acuracia,
                altitude=request.altitude,
                legenda=request.legenda,
                nome_projeto=request.nome_projeto,
                timestamp=request.data_hora,
                mini_mapa=mini_mapa
            )
        else:
            # Overlay V1 - Versão antiga (compatibilidade)
...
@app.post("/mobile/upload", response_model=MobileUploadResponse)
async def mobile_upload(request: MobileUploadRequest):
    """
    Recebe uma foto do app mobile com metadados GPS.

    O app mobile captura fotos offline e sincroniza quando
    tiver conexão. Este endpoint recebe a foto em base64
    junto com os metadados de GPS, timestamp e legenda.

    Args:
        request: MobileUploadRequest com imagem base64 e metadados

    Returns:
        MobileUploadResponse com ID da foto processada

    Raises:
        HTTPException: Se ocorrer erro no processamento
    """
    try:
        logger.info(f"Recebendo foto do mobile - local_id: {request.local_id}")

        # Valida imagem base64
        try:
            dados_imagem = base64.b64decode(request.image_base64)
        except Exception as e:
            raise HTTPException(
                status_code=400,
                detail=f"Imagem base64 inválida: {str(e)}"
        ...
        logger.info(f"Metadados: lat={meta.latitude}, lon={meta.longitude}, "
                    f"alt={meta.altitude}, dir={meta.direction}, ts={meta.timestamp}")

        # Por enquanto, apenas aceita a foto e confirma recebimento
        # No futuro: salvar em banco de dados, processar overlay, etc.

        # Opcional: aplicar overlay V2 automaticamente
        # Para uso futuro quando quiser processar no servidor

        logger.info(f"Foto mobile recebida com sucesso - id: {photo_id}, "
                    f"tamanho: {len(dados_imagem)} bytes, caption: {request.caption}")

        return MobileUploadResponse(
            success=True,
            message="Foto recebida com sucesso",
            id=photo_id,
            processed=False  # Indica que ainda não foi processada (overlay, etc)
        )

    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Erro ao receber foto mobile: {e}")
        raise HTTPException(
            status_code=500,
            detail=f"Erro ao processar upload mobile: {str(e)}"
        )
